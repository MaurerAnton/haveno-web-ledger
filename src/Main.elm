...
type Msg
    ...
    | LedgerStatusReceived Json.Decode.Value
    | LedgerAddressReceived Json.Decode.Value
    | LedgerSignResultReceived Json.Decode.Value
    | LedgerDisplayResultReceived Json.Decode.Value
    | LedgerKeyImageReceived Json.Decode.Value
    | ConnectLedger
...
update msg model =
    case msg of
        ...
        -- Ledger message handlers
        ConnectLedger ->
            ( model, requestLedgerConnect () )

        RequestLedgerAddress ->
            ( model, requestLedgerGetAddress Json.Encode.null )

        DisconnectLedger ->
            ( model, requestLedgerDisconnect () )

        LedgerStatusReceived value ->
            case model.page of
                ConnectPage connect ->
                    let
                        status =
                            Json.Decode.decodeValue (Json.Decode.field "status" Json.Decode.string) value
                    in
                    case status of
                        Ok "connected" ->
                            toConnect model ( Pages.Connect.update Pages.Connect.LedgerConnected connect )

                        Ok "disconnected" ->
                            toConnect model ( Pages.Connect.update Pages.Connect.LedgerDisconnected connect )

                        Ok "error" ->
                            let
                                errResult =
                                    Json.Decode.decodeValue (Json.Decode.field "error" Json.Decode.string) value
                                errMsg =
                                    Result.withDefault "Unknown Ledger error" errResult
                            in
                            toConnect model ( Pages.Connect.update (Pages.Connect.LedgerConnectionFailed errMsg) connect )
        ...
        LedgerAddressReceived value ->
            case model.page of
                ConnectPage connect ->
                    let
                        addrResult =
                            Json.Decode.decodeValue (Json.Decode.field "primaryAddress" Json.Decode.string) value
                    in
                    case addrResult of
                        Ok address ->
                            toConnect model ( Pages.Connect.update (Pages.Connect.LedgerAddressReceived address) connect )

                        Err _ ->
                            toConnect model ( Pages.Connect.update (Pages.Connect.LedgerConnectionFailed "Failed to decode address") connect )

                _ ->
                    ( model, Cmd.none )

        LedgerSignResultReceived _ ->
            case model.page of
                ConnectPage connect ->
                    toConnect model ( Pages.Connect.update Pages.Connect.LedgerConnected connect )
        ...
        LedgerDisplayResultReceived _ ->
            ( model, Cmd.none )

        LedgerKeyImageReceived _ ->
            ( model, Cmd.none )
...
subscriptions _ =
    Sub.batch
        ...
        , receiveLedgerStatus LedgerStatusReceived
        , receiveLedgerAddress LedgerAddressReceived
        , receiveLedgerSignResult LedgerSignResultReceived
        , receiveLedgerDisplayResult LedgerDisplayResultReceived
        , receiveLedgerKeyImage LedgerKeyImageReceived
        ]
...
-- Ledger Nano X ports (Elm → JS)
port requestLedgerConnect : () -> Cmd msg
port requestLedgerGetAddress : Json.Encode.Value -> Cmd msg
port requestLedgerSignTx : Json.Encode.Value -> Cmd msg
port requestLedgerDisplayAddress : Json.Encode.Value -> Cmd msg
port requestLedgerKeyImage : Json.Encode.Value -> Cmd msg
port requestLedgerDisconnect : () -> Cmd msg


-- Ledger Nano X ports (JS → Elm)
port receiveLedgerStatus : (Json.Decode.Value -> msg) -> Sub msg
port receiveLedgerAddress : (Json.Decode.Value -> msg) -> Sub msg
port receiveLedgerSignResult : (Json.Decode.Value -> msg) -> Sub msg
port receiveLedgerDisplayResult : (Json.Decode.Value -> msg) -> Sub msg
port receiveLedgerKeyImage : (Json.Decode.Value -> msg) -> Sub msg
