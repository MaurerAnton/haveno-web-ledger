...
import { initLedgerBridge } from './ledgerElmBridge.js'
import { isTorAvailable, isTorBrowser, generateEnvoyConfig } from './torProxy.js'
...
function initializeBrowserEnvironment () {
  try {
    ...
    // Initialize Ledger Nano X bridge (WebUSB)
    initLedgerBridge(elmapp)

    // Check Tor availability and Envoy proxy state
    isTorAvailable().then(available => {
      console.log('[Setup] Tor/Envoy proxy available:', available)
    })
  } catch (error) {
